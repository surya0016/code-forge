...
export async function GET(req:Request){
  ...
  const problemId = Number(searchParams.get("problemId")); // Extract problemId from query params

  if (!problemId) {
    return new NextResponse("Missing problemId", { status: 400 });
  }
  try{
  const problem = await db.problem.findUnique({
    ...
    console.error("[HELLO ROUTE]", error)
